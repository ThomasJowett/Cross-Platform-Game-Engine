...
void TextEditor::Duplicate()
{
	...
	UndoRecord u;
	u.mBefore = mState;
	std::string  duplicatedText;
	if (HasSelection())
	{
		duplicatedText = GetSelectedText();
		Coordinates insertAt = mState.mSelectionEnd;
		u.mAddedStart = insertAt;
		u.mAddedEnd = Coordinates(mState.mSelectionEnd.mLine + (mState.mSelectionEnd.mLine - mState.mSelectionStart.mLine), mState.mSelectionEnd.mColumn);
		InsertTextAt(insertAt, duplicatedText.c_str());
		SetSelectionStart(insertAt);
		SetSelectionEnd(u.mAddedEnd);
		SetCursorPosition(mState.mSelectionStart);
	}
	else
	{
...
bool TextEditor::SaveTextToFile(const std::filesystem::path& filepath)
{
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::GLSL()
{
	...
	if (!inited)
	{
		...
		static const char* const specials[] = {
			"gl_Position", "gl_PointSize", "gl_VertexID", "gl_InstanceID", "gl_ClipDistance", "gl_ClipVertex",
			"gl_FragCoord", "gl_FrontFacing", "gl_FragDepth"
		};
		static const char* const specialDecs[] = {
			"For writing the homogeneous vertex position", "Wrtie the size of the point to be rasterized", "Hold an integer index for the vertex", "Hold the integer index of the current primitive",
			"For controlling clipping distance", "Coordinates to be used with clipping planes", "Stores the frag coord", "If the fragment belongs to a front facing primitive", "Establishes the depth value for the fragment"
		};
		for (int i = 0; i < sizeof(specials) / sizeof(specials[0]); i++)
		{
			Identifier id;
			id.mDeclaration = specialDecs[i];
			langDef.mIdentifiers.insert(std::make_pair(std::string(specials[i]), id));
		}
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::AngelScript()
{
	...
	if (!inited)
	{
		...
		static const char* const identifiers[] = {
			"cos", "sin", "tan", "acos", "asin", "atan", "atan2", "cosh", "sinh", "tanh", "log", "log10", "pow", "sqrt", "abs", "ceil", "floor", "fraction", "closeTo", "fpFromIEEE", "fpToIEEE",
...
const TextEditor::LanguageDefinition& TextEditor::LanguageDefinition::Lua()
{
	...
	if (!inited)
	{
		...
		static const char* const identifiers[] = {
			...
			"floor", "fmod", "ult", "log", "max", "min", "modf", "rad", "random", "randomseed", "sin", "sqrt", "string", "tan", "type", "cosh", "sinh", "tanh",
			"pow", "frexp", "ldexp", "log10", "pi", "huge", "maxinteger", "mininteger", "loadlib", "searchpath", "seeall", "preload", "cpath", "path", "searchers", "loaded", "module", "require", "clock",
			"date", "difftime", "execute", "exit", "getenv", "remove", "rename", "setlocale", "time", "tmpname", "byte", "char", "dump", "find", "format", "gmatch", "gsub", "len", "lower", "match", "rep",
			"reverse", "sub", "upper", "pack", "packsize", "unpack", "concat", "maxn", "insert", "move", "sort", "offset", "codepoint", "char", "len", "codes", "charpattern",
